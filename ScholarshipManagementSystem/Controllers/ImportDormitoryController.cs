...
namespace ScholarshipManagementSystem.Controllers
{
    ...
    public class ImportDormitoryController : ApiController
    {
        ...
        public String Postgetfile()
        {
            try {
                ...
                if (row.GetCell(1).ToString() != "第一学期成绩")
                {
                    stream.Close();
                    return_msg = "表格格式不正确！第一行第二列应为：第一学期成绩";
                    return return_msg;
                }
                if (row.GetCell(2).ToString() != "第二学期成绩")
                {
                    stream.Close();
                    return_msg = "表格格式不正确！第一行第三列应为：第二学期成绩";
                    return return_msg;
                }
                ...
                while (row != null && row.GetCell(0) != null)
                {
                    ...
                    float new_score1 = float.Parse(row.GetCell(1).ToString());
                    float new_score2 = float.Parse(row.GetCell(2).ToString());
                    Dormitory s = db.Dormitorys.Find(new_user);
                    if (s == null)
                    {
                        s = new Dormitory();
                        s.Id = new_user;
