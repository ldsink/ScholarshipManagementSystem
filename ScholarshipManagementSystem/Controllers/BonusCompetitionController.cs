...
namespace ScholarshipManagementSystem.Controllers
{
    ...
    public class BonusCompetitionController : ApiController
    {
        private StudentContext db = new StudentContext();

        // GET api/BonusNormal
        public IEnumerable<BonusCompetition> GetBonusProjects()
        {
            return db.BonusCompetitions.AsEnumerable();
        }

        // PUT api/BonusNormal/5
        public HttpResponseMessage PutBonusProject(int id, BonusCompetition bonusproject)
        {
            ...
            foreach (BonusT b in bonusts) {
                if (b.Bonustype == BonusType.CompetitionBonus && b.DetailID == bonusproject.Id) {
                    b.Status = "未审核";
        ...
        public HttpResponseMessage PostBonusProject(BonusCompetition bonusproject)
        {
            ...
            if (ModelState.IsValid)
            {
                if (CheckBonusType(bonusproject.Num) != BonusType.NoExistBonus)
                    return Request.CreateErrorResponse(HttpStatusCode.BadRequest, ModelState);
                db.BonusCompetitions.Add(bonusproject);
                db.SaveChanges();
        ...
        {
            BonusCompetition bonusproject = db.BonusCompetitions.Find(id);
            ...
            foreach (BonusT b in bonusts)
            {
                // Remove BonusCompetitionDetail record.
                if (b.Bonustype == BonusType.CompetitionBonus && b.DetailID == bonusproject.Id)
                {
                    IEnumerable<BonusCompetitionDetail> bcds = db.BonusCompetitionDetails.Where(bcd => (bcd.BelongedID == b.Id));
                    foreach (BonusCompetitionDetail bcd in bcds)
                        db.BonusCompetitionDetails.Remove(bcd);
                }
                db.BonusTs.Remove(b);
            }
            // Remove BonusCompetition record.
            db.BonusCompetitions.Remove(bonusproject);
